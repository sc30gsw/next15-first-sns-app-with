...
import { cn, cr, useMediaQuery } from '@/components/ui/primitive'
...
const Inset = ({ className, ...props }: ComponentProps<'main'>) => {
  return (
    <main
...
const Item = ({
  ...
  // biome-ignore lint/style/noNonNullAssertion: initial is from JustD
...
const Rail = ({ className, ...props }: ComponentProps<'button'>) => {
  const { toggleSidebar } = useSidebar()
